...
require_relative '../game_purchase/download_key.rb'
...
class BoardGameTest < Minitest::Test
  ...
  TOPICS = ExternalTextSource::WikipediaApi::FEATURED_ARTICLE_TITLES

  TOPICS.each_with_index do |t, i|

    # # Initialization Tests
    define_method("test_game_creation_requires_a_topic_#{i}") do
      assert_raises(ArgumentError) { BoardGame.new }
    end

    define_method("test_game_creation_fails_on_obscure_topic#{i}") do
      assert_raises(ArgumentError) { BoardGame.new("buiwfubsaidfe78f7236fs7vbsds") }
    end
        
    define_method("test_game_creation_fails_on_topic_with_no_main_image#{i}") do
      assert_raises(ArgumentError) { BoardGame.new("Dungeons and Dragons") }
    end

    define_method("test_game_can_be_initiated#{i}") do
      assert (BoardGame.new(t) rescue ArgumentError), msg: "Topic was '#{t}'"
    end
    ...
    # # Download Key Tests
    # def test_download_key_can_be_decrypted
    #   TOPICS.each do |t|
    #     key_data = {
    #       :topic => t,
    #       :email => "test@test.com"
    #     }
    #     download_key = DownloadKey.new(key_data[:topic], key_data[:email]).encrypted_download_key
    #     assert_equal key_data, DownloadKey.decrypt_to_hash(download_key), msg: "Topic was '#{t}'"
    #   end
    # end
  end
